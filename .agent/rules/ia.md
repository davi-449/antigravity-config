...
- **No Frontend**: Leia `memory/ui.md`. Consulte `spec/global/features.md`. Leia as skills:
  ```
  view_file skills/ui-components/SKILL.md
  view_file skills/ui-motion/SKILL.md         ← se envolver animações
  view_file skills/deploy-production/SKILL.md ← se envolver SSR/deploy
  ```

- **No Backend**: Leia `memory/supabase.md`. Verifique o schema existente. Leia as skills:
  ```
  view_file skills/backend-patterns/SKILL.md
  view_file skills/auth/SKILL.md              ← se envolver autenticação
  ```

- **No Banco**: Leia `memory/supabase.md`. Leia as skills:
  ```
  view_file skills/database/SKILL.md
